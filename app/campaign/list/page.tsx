...
export default function AllCampaignsPage() {
    ...
    if (loading) return (
        <div className="min-h-screen bg-white dark:bg-[#0a0f1d] flex flex-col items-center justify-center">
            ...
            <p className="mt-4 text-slate-600 dark:text-gray-500 font-black uppercase tracking-widest text-xs">Syncing Feed...</p>
        </div>
    );
        ...
        <div className="min-h-screen bg-white dark:bg-[#0a0f1d] text-slate-900 dark:text-white pb-20 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12 relative z-10">
                <header className="mb-12">
                    <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-indigo-100 dark:bg-teal-500/10 border border-indigo-300 dark:border-teal-500/20 text-indigo-700 dark:text-teal-400 text-xs font-bold uppercase tracking-[0.2em] mb-4">
                        <SparklesIcon className="w-4 h-4" />
                        <span>Discover Impact</span>
                    </div>
                    <h1 className="text-5xl md:text-7xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-gray-500">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-12 p-4 bg-indigo-50 dark:bg-white/5 backdrop-blur-xl rounded-[2.5rem] border border-indigo-200 dark:border-white/10 shadow-2xl">
                    ...
                    <div className="lg:col-span-2 relative group">
                        ...
                        <input 
                            ...
                            className="w-full pl-12 pr-4 py-4 bg-white dark:bg-white/5 border border-indigo-300 dark:border-white/10 rounded-2xl focus:ring-2 focus:ring-indigo-500 dark:focus:ring-teal-500/50 outline-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-600 transition-all"
                            value={searchQuery}
                    ...
                    <div className="relative">
                        <select 
                            ...
                            className="w-full px-6 py-4 bg-white dark:bg-[#111827] border border-indigo-300 dark:border-white/10 rounded-2xl text-slate-900 dark:text-gray-300 font-bold outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-teal-500/50 appearance-none cursor-pointer"
                        >
                            {CATEGORIES.map(cat => (
                        ...
                        {(['ALL', 'ACTIVE', 'FUNDED'] as const).map((status) => (
                            <button
                                ...
                                className={`flex-1 py-2.5 text-[10px] font-black tracking-widest rounded-xl transition-all ${
                                    statusFilter === status 
                                    ? 'bg-indigo-600 dark:bg-teal-400 text-white dark:text-gray-950 shadow-lg' 
                ...
                {filteredCampaigns.length > 0 ? (
                    ...
                    <div className="text-center py-32 bg-indigo-50 dark:bg-white/5 backdrop-blur-sm rounded-[3rem] border-2 border-dashed border-indigo-200 dark:border-white/10">
                        ...
                        <p className="text-slate-600 dark:text-gray-500 mt-2 font-light">Try adjusting your filters to find more sparks.</p>
                        <button 
                            onClick={() => {setSearchQuery(''); setSelectedCategory('ALL'); setStatusFilter('ALL');}}
                            className="mt-8 text-indigo-600 dark:text-teal-400 font-black uppercase text-xs tracking-[0.2em] hover:text-indigo-700 dark:hover:text-teal-300 transition-colors"
                        >
                            Reset All Filters
                        </button>
                    </div>
