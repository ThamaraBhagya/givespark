...
export default function FeaturedCampaigns() {
  ...
  if (loading) return (
    <div className="bg-[#0a0f1d] py-32 text-center">
      <div className="inline-block w-8 h-8 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin"></div>
      <p className="mt-4 text-gray-500 font-medium tracking-widest uppercase text-xs">Loading Featured Projects</p>
    </div>
  );

  if (error) return (
    <div className="bg-[#0a0f1d] py-32 text-center text-red-400 font-mono text-sm">
      Error: {error}
    </div>
  );

  return (
    <section className="relative py-24 sm:py-32 bg-[#0a0f1d] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            <h2 className="text-sm font-black text-teal-400 tracking-[0.3em] uppercase mb-4 text-center md:text-left">
              Featured Campaigns
            </h2>
            <p className="text-4xl md:text-5xl font-black text-white tracking-tight text-center md:text-left">
              Projects Changing the <span className="bg-clip-text text-transparent bg-gradient-to-r from-teal-400 to-indigo-400">World Now</span>
            </p>
          </div>
          <Link 
            href="/campaign/list" 
            className="hidden md:flex items-center space-x-2 text-gray-400 hover:text-white transition-colors font-bold group"
          >
        ...
        <div className="mt-16 text-center md:hidden">
          <Link 
            href="/campaign/list" 
            className="inline-flex items-center space-x-2 px-8 py-3 bg-white/5 border border-white/10 text-white font-bold rounded-full hover:bg-white/10 transition-all"
          >
